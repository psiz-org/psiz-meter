...
def print_embedding_correlation(
        df_results, model_name_list, version_round_list, sim_list,
        model_pretty_list, version_pretty_list, sim_pretty_list, caption):
    """Output LaTeX table for embedding correlation.
    ...
    """
    complete_table_str = ''
    n_similarity = len(sim_list)

    df_sub = df_results[df_results['metric'] == 'correlation'].copy()
    ...
    for v in version_pretty_list:
        heading_labels_str += ' & \\multicolumn{' + str(n_similarity) + '}'
        heading_labels_str += '{|c|}{\\bf{' + v + '}}'
        for d in sim_pretty_list:
            tabular_str += 'c|'
        ...
        for v in version_round_list:
            ...
            for idx, acc in enumerate(acc_list):
                if acc == -np.inf:
                    ...
                    if idx == idx_largest:
                        # Use bold.
                        row_str += '& \\bf{{{0:.2f}}} '.format(acc)
                    else:
                        row_str += '& {0:.2f} '.format(acc)
        row_str += '\\\\'
